...
select state,round(avg(growth)*100,2) avg_growth from data1 group by state order by avg_growth desc limit 3;
...
(select c.district,c.state state,ceil(c.population/(c.sex_ratio+1)) males, floor((c.population*c.sex_ratio)/(c.sex_ratio+1)) females from
